process.env.APP_VERSION = "v4.9.21d";
...
// Tester-specific check, separate from hasPremiumAccess -- used to route Tester into
// usage tracking/limit enforcement (v4.9.22 queue item) while Admin and Pro remain
// fully exempt and untracked, per Tony's decision 2026-06-22.
function isTesterAccessCode(req) {
  const provided = normalizeAccessCode(req.headers["x-spafix-access-code"]);
  if (!provided) return false;
  return TESTER_KEYS.some(k => accessCodesMatch(provided, k));
}
...
6. If not cleared, repeat once more.`,
  ...
  'F:FJ': `⚠️ HIGH VOLTAGE WARNING\n1. Turn OFF the main circuit breaker to the spa now (the topside panel button does not cut power). Verify the tub display is completely dead.\n2. Protect Yourself: Opening the spa pack exposes live, lethal voltage. Do not touch any heavy wiring, terminal blocks, or large components on the circuit board. The only part you will touch is the low-voltage flow switch plug/pins to place your jumper.`,
};
...
- ---/Watchdog: "Your spa is showing a watchdog safety shutdown -- the control system detected an unsafe operating condition and has disabled all spa functions until service is completed and the system is reset. This one needs a closer look -- tap below and I'll pull in a deeper analysis."
- SLOC: "Your spa is showing SLOC, which means Spa Lock is active -- the control panel has been locked by a button combination on the topside panel, disabling normal control functions. This is not a fault. To deactivate it, check your owner's manual for the specific button sequence to unlock the panel -- it typically involves holding a button or combination for several seconds."
...
- "--" (Leisure Bay power interruption): "Those dashes mean power has been cut to the spa -- the control system lost its reading when power was interrupted. Check that power is restored at the breaker box electrical panel or sub-panel. Note: water temperature will be unknown until the pump has run for at least 2 minutes. If power is confirmed and the dashes remain, tap below and we'll investigate further." Route [SEQ:none].
...
BRAND MENTION RULE -- HARD STOP: If the user explicitly names a brand (e.g. "my Balboa system", "it's a Hot Spring"), that brand name MUST appear in your first sentence. This is non-negotiable. Never treat a named brand as generic. Example: user says "my Balboa HFL code" → response MUST open with "On Balboa systems, HFL means..." not "your spa has...". The brand name MUST be in the first sentence -- failure to include it is a critical error.\n\nABBREVIATION RULE -- HARD STOP: Never expand an error code abbreviation by inference. If the code is in the DB, use that definition verbatim. If the code is NOT in the DB, say "I don't have that code on file -- can you describe what's happening?" Never invent an expansion. SA = Sensor A malfunction (not "sustained absence"). DR = Dry heater fault. --- = watchdog safety shutdown -- the control system detected an unsafe operating condition and disabled all spa functions until service is completed and the system is reset (NOT a sensor fault, NOT a cold water reading -- service and reset required, no technician-attribution language). -- = power interruption / no reading -- power has been cut to the spa, temperature unknown until pump has run for 2 minutes (NOT a watchdog, NOT a fault code -- check power at breaker first, Leisure Bay specific). Use the DB definition, always.\nWATCHDOG HARD STOP: For --- watchdog codes, NEVER use the phrases "qualified technician", "requires a technician", or "manually reset by a technician" -- service and reset language is permitted, technician-attribution is not sourced and is STRICTLY FORBIDDEN.

PART TAG RULE: When you identify a specific faulty component that the user will likely need to replace (e.g. flow switch, temperature sensor, circulation pump, heater element), append a [PART:lookup_key|Display Name] tag at the end of your response, after the [SEQ:x] tag. Format: [PART:flow_switch|Flow Switch]. The lookup_key must be lowercase with underscores. Only tag parts you are confident the user needs -- do not tag generic categories. Do not tag consumables (chemicals, filters used as maintenance items). The tag is stripped from display automatically.
...
// counter back to zero on demand to retest the paywall gate without waiting for a
// real day/week rollover. Queued and built 2026-06-22 per Tony's spec.
app.post("/api/reset-usage", (req, res) => {
  if (!hasPremiumAccess(req)) {
    return res.status(403).json({ error: "Not authorized to reset usage counters." });
  }
  const resetType = req.body?.resetType;
  if (resetType !== 'daily' && resetType !== 'weekly') {
    return res.status(400).json({ error: "resetType must be 'daily' or 'weekly'." });
  }
  const clientId = getClientId(req);
  const u = getUsage(clientId);
  if (resetType === 'daily') {
...
app.post("/api/start-session", (req, res) => {
  ...
  if (proAuth.session && proAuth.session.role !== 'tester') return res.json({ allowed: true, isPro: true });
  const clientId = getClientId(req);
...
async function haikusaysSpaRelated(text) {
  try {
    ...
    // Use nativeFetch directly -- the shadowed fetch wrapper rethrows AbortErrors from upstream
    // signals, which would bypass the catch below and let off-topic messages through
...
app.post("/api/chat", async (req, res) => {
  ...
  if (lastMsg?.role === "user") {
    ...
    const check = (isSpaForm || spaSubmitted || hasSpaContext || hasDiagState || req.body.systemOverride || isShortGuidedReply(content)) ? { valid: true } : await isValidMessage(content);    if (!check.valid) {
  ...
  // Only inject [BRAND NOTE] when spa brand is confirmed -- suppress when Unknown to prevent
  // conflict with UNKNOWN BRAND RULE that causes Claude to narrate internal prompt logic in output
  const _brandIsKnown = spaMakeVal && spaMakeVal !== 'Unknown' && spaMakeVal.trim() !== '';
  if (_explicitBrand) {
  ...
  } else if (_brandIsKnown) {
    // Scenario 2: code-to-brand hint -- only when spa brand is confirmed, never when Unknown
  ...
  async function callAndProcess(tier) {
    ...
    // Strip [PART:] tags from reply before sending to client -- extract keys for partTags field
    // Client receives clean text + structured partTags array; avoids tag leak to QA evaluator
    const _partTagsOut = [];
    const _strippedReply = (correctedReply || '').replace(/\[PART:([^\|]+)\|([^\]]+)\]/g, (_, key, name) => {
      _partTagsOut.push({ key: key.trim(), name: name.trim() });
      return '';
    }).trim();
    const safeReply = _strippedReply || "I didn't quite catch that -- could you try again?";
  ...
  if (isTesterAccessCode(req)) {
    ...
    if (!qaBypass) {
      ...
      u.dailyMsgs++;
      usagePayload = { dailyMsgs: u.dailyMsgs, dailyLimit: FREE_DAILY_MSG_LIMIT, weeklySessions: u.weeklySessions, weeklyLimit: FREE_WEEKLY_SESSION_LIMIT };
    }
    try { const { reply, diagState } = await callAndProcess('admin'); res.json({ reply, diagState, spaUnknown: spaUnknownFlag, usage: usagePayload }); }
    catch (err) { res.status(500).json({ error: err.message }); }
    return;
  }
